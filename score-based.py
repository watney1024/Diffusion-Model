...
class Model(nn.Module):
    def __init__(self, device, n_steps, sigma_min, sigma_max):
        ...
        super().__init__()
        self.device = device
        self.sigmas = torch.exp(torch.linspace(start=math.log(sigma_max), end=math.log(sigma_min), steps=n_steps)).to(
            device=device)
...
dataloader = torch.utils.data.DataLoader(dataset, batch_size = 256, drop_last = True,num_workers = 6)
